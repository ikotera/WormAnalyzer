function [output, Greg] = dftregistrationGPU(buf1ft, buf2ft, usfac)
...
% Default usfac to 1
if ~exist('usfac', 'var'), usfac=1; end

% buf1ft = gpuArray(Rbuf1ft);
% buf2ft = gpuArray(Rbuf2ft);
% Gusfac = gpuArray(usfac);
Gusfac = usfac;
...
elseif usfac == 1,
    ...
    error = gather(error);
    diffphase = gather(diffphase);
    row_shift = gather(row_shift);
    col_shift = gather(col_shift);
...
% Partial-pixel shift
    ...
    mlarge = m * 2;
    nlarge = n * 2;
    CC = gpuArray.zeros(mlarge, nlarge);
...
%     n = gpuArray(n);   
    ...
    if usfac > 2,
        ...
        row_shift = round(row_shift*Gusfac)/Gusfac; 
        col_shift = round(col_shift*Gusfac)/Gusfac;     
        dftshift = fix(ceil(Gusfac*1.5)/2); %% Center of output array at dftshift+1
        % Matrix multiply DFT around the current shift estimate
        CCU = conj(dftups(buf2ft.*conj(buf1ft),ceil(Gusfac*1.5),ceil(Gusfac*1.5),Gusfac,...
            dftshift-row_shift*Gusfac,dftshift-col_shift*Gusfac))/(md2*nd2*Gusfac^2);
        % Locate maximum and map back to original pixel grid
        [CCmax, ind] = max(CCU(:));
        [rloc, cloc] = ind2sub(size(CCU), ind); 

%         rg00 = dftups(buf1ft.*conj(buf1ft),1,1,Gusfac)/(md2*nd2*Gusfac^2);
%         rf00 = dftups(buf2ft.*conj(buf2ft),1,1,Gusfac)/(md2*nd2*Gusfac^2);  
        rloc = rloc - dftshift - 1;
        cloc = cloc - dftshift - 1;
        row_shift = row_shift + rloc/Gusfac;
        col_shift = col_shift + cloc/Gusfac;    
