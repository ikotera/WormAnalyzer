...
% 	end
	...
	% Plot bars
    for ind = 1:numel(barvalues)
        handles.bars(ind) = bar(ind, barvalues(ind), width,'edgecolor','k', 'linewidth', 1);
        hold on
    end
	...
	for i = 1:numbars
        ...
        % 		x = mean(x([1 3],:));
        x = 1:3;
        handles.errors(i) = errorbar(x(i), barvalues(:,i), errors(:,i), 'k', 'linestyle', 'none', 'linewidth', 1);
	...
	if strcmp(legend_type, 'axis')
		for i = 1:numbars
			...
			for j = 1:length(xdata)
				text(xdata(j),  -0.03*ymax*1.1, bw_legend(i), 'Rotation', 45, 'fontsize', 10, 'HorizontalAlignment', 'right');
			end
		end
...
% 	set(gca, 'xticklabel', groupnames, 'box', 'off', 'ticklength', [0 0], 'fontsize', 12, 'xtick',1:numgroups, 'linewidth', 2,'xgrid','off','ygrid','off');
    set(gca, 'xticklabel', groupnames, 'ticklength', [0 0], 'xtick',1:3, 'xgrid','off', 'Position', [.13 .12 .82 .80]);
