...
for i = 1:nRows
    for j = 1:nCols 
        %%%% ADDED BY PS %%%%
        if isequal(class(hAxGrid(i,j)),'matlab.graphics.GraphicsPlaceholder')
            spMat = flipud(reshape(1:nRows*nCols,nCols,nRows)');
            spNum = spMat(i,j);
            spToDelete = subplot(nRows,nCols,spNum);
            spsToDelete = [spsToDelete spToDelete];
            hAxGrid = getappdata(gcf,'SubplotGrid');
        end      
        %%%% END ADDED BY PS %%%%
...
%%%% ADDED BY PS %%%%
for i = 1:numel(spsToDelete)
    delete(spsToDelete(i));
end
%%%% END ADDED BY PS %%%%
set(gcf,'units',u1)
