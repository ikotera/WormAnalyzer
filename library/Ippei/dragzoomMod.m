...
%==========================================================================
    function SetFigureCallbacks2D()
        ...
        set(hCont, 'MouseWheelMovedCallback', {@WindowScrollWheelFcn2D});
    end
%--------------------------------------------------------------------------
...
%                 prt(clickType);
        ...
        if ~strcmp(clickType, 'open')
            isRightClick = false;           % if the second click didn't immediately follow the right click
        end
        ...
        switch clickType
            ...
            case 'open'
                if isRightClick
                    hFunctions.resetTime();
%                     prt('Right Double Click');
                    isRightClick = false;
                elseif fIsMagnifierOn
...
%==========================================================================
    function WindowButtonMotionCallback2D(src, evnt)
        ...
        hFunctions.mouseMoved(src, evnt);
    end
%--------------------------------------------------------------------------
...
%==========================================================================
    function WindowScrollWheelFcn2D(src, evnt)        %#ok
        ...
        if evnt.getModifiers == 2 % if Ctrl key is pressed
%         if false
            % Update Zoom Info
            ...
            verScrollCount = evnt.getWheelRotation;
            ...
            % Perform scrolling function in variable_analysis
            hFunctions.scrollByWheel([], evnt); 
        end
    end
%--------------------------------------------------------------------------
...
%             unitDefault = get(get(h, 'Parent'), 'Units');
%             set(get(h, 'Parent'), 'Units', 'pixels');
            AxesInfo(i).xlim = get(h, 'XLim');
            AxesInfo(i).ylim = get(h, 'YLim');
%             set(get(h, 'Parent'), 'Units', unitDefault);
