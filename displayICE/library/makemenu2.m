function handles = makemenu2(fig, labels, calls, tags)
...
for k = 1:num_objects
  ...
  if (labelStr(1) == '-')
    ...
    h = uimenu(remember_handles(length(remember_handles)), ...
        'Label', labelStr, 'Accelerator', acc, 'Callback', calls{k, 1}, ...
        'Separator',separator,...
        'Tag', tagStr);
%     h = uimenu(remember_handles(length(remember_handles)), ...
%         'Label', labelStr, 'Accelerator', acc, 'Callback', {hFunctions.changeLayer, 'red'}, ...
%         'Separator',separator,...
%         'Tag', tagStr);
